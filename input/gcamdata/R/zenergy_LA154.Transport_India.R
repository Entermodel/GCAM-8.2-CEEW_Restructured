...
module_india_LA154.Transport <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    Transportation_Fuels <- c("Coal", "Electric", "Liquids", "NG", "FCEV", "BEV","Hydrogen","Hybrid Liquids")
    ...
    Transportation_energy_consumption <- A54.india_core_changed %>%
      ...
      ungroup() %>%
      unique() %>%
      left_join_error_no_match(trnIndia_GHG_mapping, by = c("fuel", "mode", "UCD_sector","UCD_technology"))
