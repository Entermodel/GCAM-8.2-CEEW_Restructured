...
module_gcamindia_LA144.Residential <- function(command, ...) {
  ...
  MODULE_INPUTS <-
    c(FILE = "gcam-india/A44.india_state_pcflsp_m2_res_rural",
  ...
  MODULE_OUTPUTS <-
    c("L144.india_state_flsp_bm2_res",
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    L144.india_state_flsp_bm2_res_rural <- L100.india_state_pop_ruralurban %>%
      filter (sector %in% 'resid rural') %>%
    ...
    #for urban
    L144.india_state_flsp_bm2_res_urban <- L100.india_state_pop_ruralurban %>%
      filter (sector %in% 'resid urban')%>%
      filter(year %in% HISTORICAL_YEARS) %>% #which are needed
      filter(year %in% MODEL_BASE_YEARS) %>%
    ...
    # ===================================================
     ...
     A44.india_state_in_EJ_res_F_spaceheating_Y_long <-  A44.india_state_in_EJ_res_F_spaceheating_Y %>%
       pivot_longer(
         cols = where(is.numeric),  # Pivots all numeric columns
         names_to = "year",
         values_to = "value"
       )
     A44.india_state_in_EJ_res_F_spacecooling_Y_long <-  A44.india_state_in_EJ_res_F_spacecooling_Y %>%
       pivot_longer(
         cols = where(is.numeric),  # Pivots all numeric columns
         names_to = "year",
         values_to = "value"
       )
     A44.india_state_in_EJ_res_F_cooking_Y_long <-  A44.india_state_in_EJ_res_F_cooking_Y %>%
       pivot_longer(
         cols = where(is.numeric),  # Pivots all numeric columns
         names_to = "year",
         values_to = "value"
       )
     A44.india_state_in_EJ_res_F_lighting_Y_long <-  A44.india_state_in_EJ_res_F_lighting_Y %>%
       pivot_longer(
         cols = where(is.numeric),  # Pivots all numeric columns
         names_to = "year",
         values_to = "value"
       )
     A44.india_state_in_EJ_res_F_apploth_Y_long <-  A44.india_state_in_EJ_res_F_apploth_Y %>%
       pivot_longer(
         cols = where(is.numeric),  # Pivots all numeric columns
         names_to = "year",
         values_to = "value"
     ...
     L144.india_state_in_EJ_res_F_U_Y_unscaled <- bind_rows(A44.india_state_in_EJ_res_F_spaceheating_Y_long,
                                                            A44.india_state_in_EJ_res_F_spacecooling_Y_long,
                                                            A44.india_state_in_EJ_res_F_cooking_Y_long,
                                                            A44.india_state_in_EJ_res_F_lighting_Y_long,
                                                            A44.india_state_in_EJ_res_F_apploth_Y_long)
